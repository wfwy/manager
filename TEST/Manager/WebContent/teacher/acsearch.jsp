...
String sql = "select score.id,score.s_num,score.name,item.type,score.itme,item.score,score.remark FROM score,item where score.itme=item.item and item.type='校内活动' ";
...
<tr>
	<td>
        ...
        <% if(intPage>1){ %>
            <a href="acsearch.jsp?page=<%=intPage-1%>">上一页</a>
        <% }else{ %>
            上一页
        <%}%>
        <% if(intPage<intPageCount){ %>
            <a href="acsearch.jsp?page=<%=intPage+1%>">下一页</a>
        <% }else{ %>
            下一页
        <%}%>
