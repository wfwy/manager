...
<script src="../js/activity.js" charset="utf-8"></script>
...
<div align="center">
<form action="activitysearch.jsp"> 
...
String sql = "select score.id,score.s_num,score.name,item.type,score.itme,item.score,score.remark FROM score,item where score.itme=item.item and item.type='校内活动' and score.name='"+name+"'";
...
if(intPageCount>0) {
  ...
  while(i<intPageSize &&!rs.isAfterLast()){
		...
		out.print("<td><a href='updatescore.jsp?id="+rs.getInt("score.id")+"&type=1'onclick='javascript:return del();' class='layui-btn'><i class='layui-icon'></i>修改</a></td>");
		out.print("</tr>");
		rs.next();
        i++;
        }
...
<tr>
	<td>
        ...
        <% if(intPage>1){ %>
            <a href="activitysearch.jsp?page=<%=intPage-1%>">上一页</a>
        <% }else{ %>
            上一页
        <%}%>
        <% if(intPage<intPageCount){ %>
            <a href="activitysearch.jsp?page=<%=intPage+1%>">下一页</a>
        <% }else{ %>
            下一页
        <%}%>
