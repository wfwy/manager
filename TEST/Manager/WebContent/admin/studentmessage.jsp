...
while(i<intPageSize &&!rs.isAfterLast()){
		...
		out.print("<td><a href='student_del?s_num="+rs.getString("s_num")+"' onclick='javascript:return del();' class='layui-btn'><i class='layui-icon'></i>删除</a></td>");
		out.print("<td><a href='s_update.jsp?s_num="+rs.getString("s_num")+"' class='layui-btn'><i class='layui-icon'></i>修改</a></td>");
...
<tr>
	<td>
        ...
        <% if(intPage>1){ %>
            <a href="studentmessage.jsp?page=<%=intPage-1%>">上一页</a>
        <% }else{ %>
            上一页
        <%}%>
        <% if(intPage<intPageCount){ %>
            <a href="studentmessage.jsp?page=<%=intPage+1%>">下一页</a>
        <% }else{ %>
            下一页
        <%}%>
