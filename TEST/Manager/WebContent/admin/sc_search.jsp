...
while(i<intPageSize &&!rs.isAfterLast()){
		...
		out.print("<td><a href='score_typemanagement?id="+rs.getInt("id")+"' onclick='javascript:return del();' class='layui-btn'><i class='layui-icon'></i>删除</a></td>");
		out.print("</tr>");
		rs.next();
        i++;
        }
...
<tr>
	<td>
        ...
        <% if(intPage>1){ %>
            <a href="sc_search.jsp?page=<%=intPage-1%>">上一页</a>
        <% }else{ %>
            上一页
        <%}%>
        <% if(intPage<intPageCount){ %>
            <a href="sc_search.jsp?page=<%=intPage+1%>">下一页</a>
        <% }else{ %>
            下一页
        <%}%>
